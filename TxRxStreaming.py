...
def rx(rxStreamer,rxSamplesLen,loops,usrp,event):
    # Receive Samples
    samples = np.zeros(loops*rxSamplesLen, dtype=np.complex64)
    event.set()
    ...
    for i in range(loops*rxSamplesLen//1000):
        rxStreamer.recv(recv_buffer, metadata)
        samples[i*1000:(i+1)*1000] = recv_buffer[0]
    return samples,rxTimeHost
    

def tx(txStreamer,txData,loops,usrp,event):
...
def txrx(loops,txData,txStreamer,rxStreamer,usrp,rxTimeQueue,txTimeQueue,rxQueue):
    ...
    for i in range(1):
        thread1 = NewThread(target=tx,args=(txStreamer,txData,loops,usrp,event))
        thread2 = NewThread(target=rx,args=(rxStreamer,rxSamplesLen,loops,usrp,event))
...
if __name__ == '__main__':
    ...
    path = r"./2024_1_16_sigGen10Smallguard256128CP.mat"
    data = mat73.loadmat(path)

    sync = 0 #显示Sync
    comm = 0 #显示Constellation
    sensing = 0 #显示Doppler
    ...
    sample_rate = 50e6 # Hz
    # num_samps = int(2e6) # number of samples received
    ...
    # p1 = Process(target=txrx,args=(txData,rxStreamer,center_freq,sample_rate,txGain,usrp,rxTimeQueue,txTimeQueue,rxQueue))
    log.info("启动USRP同步收发线程")
    p1.start()
    # c1 = threading.Thread(target=consumer,args=(txData,Nzp,Ncp,kv,ltau,preamble,sample_rate,rxTimeQueue,txTimeQueue,rxQueue))
    # c1.daemon = True
    # c1.start()
    ...
    while True:
        ...
        # rxSamples.tofile('demo.dat')

        scipy.io.savemat('data.mat',{'rxSamples':rxSamples})
